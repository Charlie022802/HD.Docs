...
- 升完之後**還有兩處會壞**，都在 `hd-web-server`（見三之三），需要與同事協調：
  報告格式清單端點、以及一支**不在版控裡的 proc**（`insert_routing_job`）。
...
2. **`insert_routing_job`**：這支該進版控（補一支 migration），還是改呼叫 `insert_study_job`？
   **在解決之前，最新版 hd-web-server 不能部署到任何新建的資料庫上。**
