...
- **⚠️ 目標建議從 `2.0.38` 拉到 `2.0.41`**（2026-08-31 決定：現在不手動補，留到升級時一起做）。
  多的三版都是小改動：`2.0.39`（兩個作廢的標記）、`2.0.40`（`HD_IDENTITY_MIRROR` 投影表）、
  `2.0.41`（`HD_DEVICE_LICENSE.APP_VERSION` 從 `varchar(32)` 加寬到 64）。
  **不現在手動套 `2.0.41` 的 ALTER**，理由是那會再製造一次「版本號與實際結構對不起來」——
  正是這份文件在講的分岔問題，而且 2026-08-31 已經因此寫錯過一條部署前提。
  不套的代價只有「主控台上版本字串被截幾個字」，客戶端有 `22001` 退回 32 的退路。
- **若瑟的 `HD_DEVICE_LICENSE` 已經存在**（2026-08-19 前後單獨補的，沒走更新鏈），
  而版本號仍是 `2.0.22`。**判斷這張表在不在要用 `to_regclass`，不要用版本號** ——
  同期的 `SITE`／`PACKAGE_JOB`／`HD_USER_AUDIT_LOG`／`UPS_WORKITEM` 都還沒有，
  所以確實是只補了這一張。跑到 `2.0.29` 時那段是 `CREATE TABLE IF NOT EXISTS`
  ＋`ADD COLUMN IF NOT EXISTS`，重跑是安全的。
...
> 最後兩列是 **2026-08-26 做 JIT 佈建時撞到的**（`INSERT` 寫了 `ENABLE` → `42703 column does not exist`）。
...
### ✅ 已補完（2026-08-26，Database repo `a971fa3`）

一路往上撞完，補掉兩類缺口，**現在從若瑟的原始 dump、零手動修改，能一路跑到 `2.0.38`**：
...
### 行尾也會讓 prosrc 修補失敗（這顆很不直覺）

`2.0.33`／`2.0.36`／`2.0.37` 用「讀出 `prosrc` → 找錨點 → 字串取代 → 重建函式」的方式修補
既有的 proc。而 **`prosrc` 會原樣保留建立當下的行尾**。

若那支 proc 當初是從 **CRLF** 的檔案跑進去的，用 `
` 寫的錨點就對不上，
而錯誤訊息會說「proc 內容與基準不同，請人工確認」—— **完全看不出真正的原因是行尾**。

本 repo 的 `.sql` 有 **29/41 在工作目錄是 CRLF**：
`.gitattributes` 的 `eol=lf` 是在檔案已經檢出之後才加的
（`git ls-files --eol` 顯示 `i/lf w/crlf`）。
所以「從這份 Windows 工作目錄跑 migration」與「從 Linux 跑」會得到不同的 `prosrc`。

已在三支腳本裡先 `replace(v_src, chr(13), '')`，順帶讓改寫後的 proc 統一成 LF。

### 長期做法

1. **新的結構變更一律經由 migration 腳本**，不要直接改資料庫。
2. **驗證至少要在一份「舊站台的 dump」上跑一次**，不能只在 `.191` 上跑 ——
   `.191` 上那些物件本來就在，所以永遠撞不到。
3. 本次證明這種驗證很便宜：**一份 schema dump ＋ 一個 podman 容器，十分鐘跑完整條鏈，
   失敗零成本、完全不碰生產機**。這應該變成每次改 DB 的例行動作。
