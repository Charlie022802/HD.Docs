...
- [ ] **Viewer 去 DB 化 + 新舊系統相容（設計定案 2026-08-25，正本＝[systems/viewer.md](systems/viewer.md)）**。
  ...
  **相容性住在 ViewerWebApi**，Viewer 只認一個位址、一套認證。
  ...
  - 視訊已確認**不是缺口**（2026-09-01 修正措辭）：新系統**收得下** MPEG-4 instance，只是進檔
    不做轉換（REQ-008 停用的 DicomToVideo 是把 multiframe **轉成** DicomMpeg4 的流程，與這裡無關）；
    舊版本來就沒有。**取像這半 2026-09-01 變了**：DicomWeb `alpha.24` 起 `Accept: video/mp4`
    直接送出封裝的串流（PS3.18 表 8.7.4-1 的 Video 類，標準行為，送出前檢查容器格式），
    所以看片端日後若要播視訊，不需要預轉、也不需要直連 DB。
