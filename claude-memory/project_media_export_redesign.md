...
**🔑 REQ-020 歷史清單+過期標記完成(2026-08-21,三層上線驗證):DB `v2.0.32`(.191)+pacs `2.0.12`(.191 worker)+Export `0.1.0-alpha.14`(.199)。**
新增 `GET /export/packages`(游標分頁/狀態多選/`CREATED_AT` 區間),單筆端點不動只多回 `createdAt`/`modifiedAt`(proc 本來就有回,只是沒映射)。
- **游標不用 offset**:歷史清單一直有新 job 插到最前面,offset 往下捲會重複或漏掉。**排序用 `jobId DESC` 不用 `createdAt`** —— 既有索引 `("REQUESTED_BY","JOB_ID" DESC)` 正好吃得到。
- **過期標記選「清理時改狀態」而不是「查詢時 stat 檔案」**:`downloadReady` 原本只看 DB 欄位、從不確認檔案在不在,而 worker 按時間刪檔且完全不碰 DB → 清單一路顯示可下載、點了才失敗。**「檔案沒了」該由做這件事的人記錄,不是讓每個讀取者事後去猜**(查詢時 stat 的成本攤給所有查詢、NAS 更慢、還有 TOCTOU)。順序刻意是**先改 DB、後刪檔**:反過來刪檔成功但 DB 失敗就回到會騙人的狀態。
- **`UPDATE ... RETURNING` 給的是更新後的值** —— 要回傳給 worker 的 `RESULT_PATH` 正好被這次更新設成 NULL,路徑必須在 UPDATE 之前先撈進 CTE。冒煙測試特地斷言「回傳的 50 筆 resultPath 都不是 null」:寫錯的話只有這條會抓到,其他斷言全會過。
- **兩張 job 表撞號**(動清理時才發現,比清理本身嚴重):`EXPORT_JOB.JOB_REF` 與 `PACKAGE_JOB.JOB_ID` 是兩個獨立序列,而產出目錄只用編號命名,worker 開工前又會 `Directory.Delete` 砍掉同名目錄 → 後開工的把前一個的產出刪光而前一個仍是 `ready`,**撞的當下沒有任何徵兆**。修法:新表產出移到 `burnTemp/package/`。連帶 legacy 的時間掃描要跳過那個容器目錄,否則七天後會把裡面沒過期的一起刪光。
- **保留天數改設定** `packageRetainDays`(預設 7,原本寫死 2 天)。
- 驗證手法:把保留天數暫調成 2 天,worker 下一輪就把 50 筆 `ready` 標成 `expired`,當天建的沒被碰 —— 一次證明程式碼有執行、讀得到設定、只動該動的。
- **⏰ 2026-08-28 要回頭確認** `burnTemp/package/` 沒被 legacy 掃描誤刪(失敗要滿七天才現形),見 docs/todo.md。
