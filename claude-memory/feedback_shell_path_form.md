...
**更好的做法:比 SSH 主機金鑰,不必登入(2026-09-01)。**
```
ssh-keyscan -t ed25519 <ip> | ssh-keygen -lf -      # 現在這台
grep <ip> ~/.ssh/known_hosts | ssh-keygen -lf -     # 以前連過的那台
```
主機金鑰是各機器安裝時產生的,**兩台不同的機器不可能一樣**,而且不會因為服務停掉、
磁碟被改而變。關鍵是**它不需要登入**——而「懷疑連錯機器」的時候,上面那幾個
machine-id/MAC/uptime 的做法都得先連進去,登入本身就已經連上去了。
(`uptime -s` 仍然好用,只是要登入:我們那台 `2026-07-15`,另一台 `2025-12-24`。)

**⚠️ 別從外部探測推論服務狀態(2026-09-01,我今天犯的)。**
外面連不到 `.163:5100`,我就說「服務掛了」,還叫使用者去查 systemd。
**實際上服務 18 小時沒斷過、5100 有在聽、firewalld 永久與執行中規則都放行**,
只是網路路徑有一段時間不通,後來自己恢復。
正確的說法是「**從外面連不到**」,那有很多種原因,「服務停了」只是其中一種,
而且是唯一需要登入才確認得了的那一種。
**同一天被同一個工具誤導兩次**:`/dev/tcp` 的埠位探測對這台不穩,
早上也回報過「有東西在聽但不回 HTTP」,後來證實是誤判。
連測三次都通才算通,單次「不通」不足以下結論。
