...
**rendered 非影像路徑的匿名缺口（alpha.27，2026-09-01 修＋實測）：** 四條非影像 rendered 路徑（PDF／SR／視訊／ECG）**從一開始就沒套匿名規則**，而其中三條會送出病人資訊——ECG 與 SR 的姓名／病歷號**是我們自己畫上去的**，封裝 PDF 是原樣轉送。修法各異：ECG 用現成的 `EcgViewOptions.Anonymize`（遮罩字串是 `(anonymised)`，12 字元）、SR 先把規則套在 dataset 再渲染、**封裝 PDF 改不了 → 403 `anonymisation_not_possible`**（代價：綁匿名規則的金鑰從此完全拿不到封裝 PDF，要取得就得另配不綁規則的金鑰，那是要人拍板的決定）。**快取鍵必須帶匿名旗標**，否則一般金鑰先取過的那份會被匿名金鑰拿到——無聲的外洩。
教訓：**新增輸出路徑時，橫切關注（匿名／稽核分類／快取鍵）不會自動跟上**。這條是去查「能不能開排版參數」時看到 `EcgViewOptions.Anonymize` 才回頭發現的，不是任何測試抓到的——因為測試金鑰沒綁規則，所有測試都不會顯示它。驗證有鑑別力的關鍵是「今早那個病歷號還在嗎 → False」，同一份資料的前後對照，不是只看「輸出裡沒有可疑字串」。
