...
**~~看片端授權的強制模式需要 `v2.0.29` 以上~~ ✅ 2026-08-31 更正:不成立。**
`HD_DEVICE_LICENSE` 雖然是 `db_update_v2.0.29.sql` 建的,但**若瑟那張表已經存在**
(2026-08-19 前後單獨補的,沒走更新鏈;同期的 `SITE`/`PACKAGE_JOB`/`HD_USER_AUDIT_LOG`/
`UPS_WORKITEM` 都還沒有)。所以強制版對若瑟**不會**踩到「無法註冊」那條路。
**判準是 `to_regclass` 不是版本號** —— `2.0.22` 這個數字在這裡會給出假的否定,
又一次 [[project_db_chain_drift]]。

不過若瑟的 `APP_VERSION` 仍是 `varchar(32)`(加寬在 `v2.0.41`,升級計畫的目標是 `2.0.38`),
所以客戶端「送 64 → 22001 → 退回 32」那條退路在若瑟會實際被走到。詳 [[project_viewer_license]]。
預演環境待定:
